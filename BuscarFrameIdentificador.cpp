...
wxBEGIN_EVENT_TABLE(BuscarFrameIdentificador, wxFrame)
EVT_BUTTON(wxID_OK, BuscarFrameIdentificador::OnBuscar)
EVT_BUTTON(wxID_CANCEL, BuscarFrameIdentificador::OnCancelar)
EVT_CLOSE(BuscarFrameIdentificador::OnClose)
EVT_GRID_CELL_LEFT_DCLICK(BuscarFrameIdentificador::OnGridCellDClick)
EVT_GRID_SELECT_CELL(BuscarFrameIdentificador::OnGridSelectCell)
EVT_CHAR_HOOK(BuscarFrameIdentificador::OnKeyDown)  // Capturar la tecla ESC
wxEND_EVENT_TABLE()

BuscarFrameIdentificador::BuscarFrameIdentificador(wxWindow* parent, const wxString& title, const wxPoint& pos, const wxSize& size)
...
BuscarFrameIdentificador::~BuscarFrameIdentificador() {
    CerrarConexion();
}

void BuscarFrameIdentificador::OnBuscar(wxCommandEvent& event)
{
...
void BuscarFrameIdentificador::OnCancelar(wxCommandEvent& event) {
...
void BuscarFrameIdentificador::OnClose(wxCloseEvent& event) {
    // Deshabilitar temporalmente el manejador de eventos para evitar ciclos o múltiples llamadas
    this->Unbind(wxEVT_CLOSE_WINDOW, &BuscarFrameIdentificador::OnClose, this);
    ...
    if (!IsBeingDeleted()) {
        ...
        if (mainFrameParent != nullptr && !mainFrameParent->IsBeingDeleted()) {
            CreacionIdentificadorFrame* creacionFrame = dynamic_cast<CreacionIdentificadorFrame*>(mainFrameParent);
    ...
    // No volver a enlazar el manejador de eventos una vez que la ventana ha sido cerrada
    // Esto previene reentradas innecesarias de eventos durante el cierre.

    // Permitir que wxWidgets continúe con el proceso de cierre
    event.Skip();
}

void BuscarFrameIdentificador::OnKeyDown(wxKeyEvent& event)
{
...
void BuscarFrameIdentificador::OnGridCellDClick(wxGridEvent& event)
{
    ...
    // Cast a la ventana principal (CreacionIdentificadorFrame) y pasar los valores seleccionados
    CreacionIdentificadorFrame* creacionFrame = dynamic_cast<CreacionIdentificadorFrame*>(mainFrameParent);
    if (creacionFrame != nullptr)
    {
        creacionFrame->BuscarOperacionIdentificador(operacionID, descripcion);  // Pasar OperacionID y Descripción
...
void BuscarFrameIdentificador::ActualizarEstadoBotonesNavegacion(CreacionIdentificadorFrame* creacionFrame) {
    if (creacionFrame != nullptr) {
...
void BuscarFrameIdentificador::OnGridSelectCell(wxGridEvent& event)
{
    int row = event.GetRow();
    UpdateRowLabels(row);
    event.Skip();
...
void BuscarFrameIdentificador::UpdateRowLabels(int selectedRow)
{
...
bool BuscarFrameIdentificador::ConectarBaseDeDatos()
{
...
void BuscarFrameIdentificador::BuscarOperacionesEnBaseDeDatos(const wxString& operacion, const wxString& descripcion)
{
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);

    // Construir la consulta SQL
    wxString query = L"SELECT Operacion, OperacionID, Descripcion FROM Identificador WHERE 1=1";
...
void BuscarFrameIdentificador::CerrarConexion() {
