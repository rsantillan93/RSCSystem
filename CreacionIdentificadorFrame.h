...
#include <sqlext.h>
#include "MainFrame.h"
#include "BuscarFrameIdentificador.h"  // Incluir la declaración de BuscarFrameIdentificador

class CreacionIdentificadorFrame : public wxFrame {
public:
    // Constructor y Destructor
    CreacionIdentificadorFrame(const wxString& title, const wxPoint& pos, const wxSize& size, MainFrame* parent, const wxString& usuarioAutenticado);
    ~CreacionIdentificadorFrame();

    // Métodos públicos
    bool HayRegistrosEnTabla();
    void OnEditar(wxCommandEvent& event);

    // Método para buscar y cargar datos del identificador por OperacionID
    void BuscarOperacionIdentificador(const wxString& operacionID, const wxString& descripcion);
