...
#include <sqlext.h>
#include "MainFrame.h"
#include "BuscarFramePresentacion.h"  // Incluir la declaración de BuscarFramePresentacion

class CreacionPresentacionFrame : public wxFrame {
public:
    // Constructor y Destructor
    CreacionPresentacionFrame(const wxString& title, const wxPoint& pos, const wxSize& size, MainFrame* parent, const wxString& usuarioAutenticado);
    ~CreacionPresentacionFrame();

    // Métodos públicos
    bool HayRegistrosEnTabla();
    void OnEditar(wxCommandEvent& event);

    // Método para buscar y cargar datos de la presentación por OperacionID
    void BuscarOperacionPresentacion(const wxString& operacionID, const wxString& descripcion);
