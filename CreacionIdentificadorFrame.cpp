...
wxBEGIN_EVENT_TABLE(CreacionIdentificadorFrame, wxFrame)
EVT_BUTTON(ID_BTN_NUEVO, CreacionIdentificadorFrame::OnNuevo)
EVT_BUTTON(ID_BTN_GUARDAR, CreacionIdentificadorFrame::OnGuardar)
EVT_BUTTON(ID_BTN_EDITAR, CreacionIdentificadorFrame::OnEditar)
EVT_BUTTON(ID_BTN_CANCELAR, CreacionIdentificadorFrame::OnCancelar)
EVT_BUTTON(ID_BTN_ELIMINAR, CreacionIdentificadorFrame::OnEliminar)
EVT_BUTTON(ID_BTN_BUSCAR, CreacionIdentificadorFrame::OnBuscar)
EVT_BUTTON(ID_BTN_IZQUIERDA, CreacionIdentificadorFrame::OnIzquierda)
EVT_BUTTON(ID_BTN_DERECHA, CreacionIdentificadorFrame::OnDerecha)
EVT_BUTTON(ID_BTN_EXPORTAR, CreacionIdentificadorFrame::OnExportarExcel) // Añadido para Exportar Excel
EVT_TEXT(wxID_ANY, CreacionIdentificadorFrame::OnDescripcionChange)
EVT_CHAR_HOOK(CreacionIdentificadorFrame::OnKeyDown)
EVT_CLOSE(CreacionIdentificadorFrame::OnClose)
wxEND_EVENT_TABLE()

CreacionIdentificadorFrame::CreacionIdentificadorFrame(const wxString& title, const wxPoint& pos, const wxSize& size, MainFrame* parent, const wxString& usuario)
...
CreacionIdentificadorFrame::~CreacionIdentificadorFrame() {
    CerrarConexion();
}

void CreacionIdentificadorFrame::OnExportarExcel(wxCommandEvent& event) {
    try {
        ...
        wxString exportPath = folderPath + "\\identificador.xlsx";  // Cambiado el nombre a identificador.xlsx
        ...
        // Consultar la base de datos para obtener los datos de la tabla Identificador
        SQLHSTMT hStmt = NULL;
        SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
        wxString query = L"SELECT OperacionID, Descripcion FROM Identificador;";
...
void CreacionIdentificadorFrame::OnDescripcionChange(wxCommandEvent& event) {
...
wxString CreacionIdentificadorFrame::BuscarDescripcionSimilar(const wxString& descripcion) {
    const int UMBRAL_SIMILITUD = 3;

    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = L"SELECT Descripcion FROM Identificador;";
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
wxString CreacionIdentificadorFrame::NormalizarCadena(const wxString& cadena) {
...
bool CreacionIdentificadorFrame::EsNombreRepetitivo(const wxString& descripcion) {
...
int CreacionIdentificadorFrame::CalcularDistanciaLevenshtein(const wxString& source, const wxString& target) {
...
bool CreacionIdentificadorFrame::HayRegistrosEnTabla() {
    SQLHSTMT hStmt = NULL;
    if (hDbc == NULL) {
    ...
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = L"SELECT COUNT(*) FROM Identificador;";
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::OnNuevo(wxCommandEvent& event) {
    EnableInputs(true);
    ClearInputs();
    lblSimilitud->SetLabel("");
    isNewOperation = true;
...
bool CreacionIdentificadorFrame::OperacionExists(int operacion) {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = wxString::Format(L"SELECT COUNT(*) FROM Identificador WHERE Operacion = %d;", operacion);
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::OnGuardar(wxCommandEvent& event) {
    lblSimilitud->SetLabel("");
    ...
    if (isEditing) {
        query = L"UPDATE Identificador SET Descripcion = ?, UsuarioID = ?, Ordenador = ? WHERE OperacionID = ?;";
    }
    else {
        query = L"INSERT INTO Identificador (Operacion, OperacionID, Descripcion, UsuarioID, Ordenador) VALUES (?, ?, ?, ?, ?);";
    }
    ...
    if (isEditing) {
        ...
        SQLBindParameter(hStmt, 4, SQL_PARAM_INPUT, SQL_C_WCHAR, SQL_WVARCHAR, operacionIDStr.Length(), 0, (SQLPOINTER)operacionIDStr.wc_str(), 0, NULL);
    }
    else {
        SQLBindParameter(hStmt, 1, SQL_PARAM_INPUT, SQL_C_LONG, SQL_INTEGER, 0, 0, &operacionValue, 0, NULL);
...
void CreacionIdentificadorFrame::OnEditar(wxCommandEvent& event) {
...
void CreacionIdentificadorFrame::OnBuscar(wxCommandEvent& event) {
    lblSimilitud->SetLabel("");

    BuscarFrameIdentificador* buscarFrame = new BuscarFrameIdentificador(this, "Buscar Identificadores", wxDefaultPosition, wxSize(750, 500));
    buscarFrame->CentreOnParent();
    buscarFrame->Show(true);
    Disable();
}

void CreacionIdentificadorFrame::OnCancelar(wxCommandEvent& event) {
    this->Unbind(wxEVT_CLOSE_WINDOW, &CreacionIdentificadorFrame::OnClose, this);
    ...
    if (isNewOperation) {
        ...
        if (operacionID == 0 || !HayRegistrosEnTabla()) {
            ...
            this->Bind(wxEVT_CLOSE_WINDOW, &CreacionIdentificadorFrame::OnClose, this);
            return;
        }
        else {
...
void CreacionIdentificadorFrame::BuscarOperacionIdentificador(const wxString& operacionID, const wxString& descripcion) {
    ...
    wxString query = wxString::Format(L"SELECT Operacion, OperacionID, Descripcion FROM Identificador WHERE OperacionID = '%s';", operacionID);
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
int CreacionIdentificadorFrame::ObtenerSiguienteOperacion() {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = L"SELECT ISNULL(MAX(Operacion), 0) + 1 FROM Identificador;";
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::CargarUltimoRegistro() {
...
int CreacionIdentificadorFrame::ObtenerPrimerOperacion() {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = L"SELECT MIN(Operacion) FROM Identificador;";
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
int CreacionIdentificadorFrame::ObtenerUltimaOperacion() {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = L"SELECT MAX(Operacion) FROM Identificador;";
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::OnIzquierda(wxCommandEvent& event) {
    int currentID = wxAtoi(txtOperacionID->GetValue());
...
void CreacionIdentificadorFrame::OnDerecha(wxCommandEvent& event) {
    int currentID = wxAtoi(txtOperacionID->GetValue());
...
int CreacionIdentificadorFrame::ObtenerOperacionAnterior(int currentID) {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = wxString::Format(L"SELECT MAX(Operacion) FROM Identificador WHERE Operacion < %d;", currentID);
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
int CreacionIdentificadorFrame::ObtenerOperacionSiguiente(int currentID) {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = wxString::Format(L"SELECT MIN(Operacion) FROM Identificador WHERE Operacion > %d;", currentID);
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::OnEliminar(wxCommandEvent& event) {
    lblSimilitud->SetLabel("");
    ...
    try {
        SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
        wxString query = L"DELETE FROM Identificador WHERE OperacionID = ?;";
        ret = SQLPrepare(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
void CreacionIdentificadorFrame::OnClose(wxCloseEvent& event) {
...
void CreacionIdentificadorFrame::OnKeyDown(wxKeyEvent& event) {
...
void CreacionIdentificadorFrame::EnableInputs(bool enable) {
    txtOperacionID->Enable(false);
...
void CreacionIdentificadorFrame::ClearInputs() {
    txtOperacionID->Clear();
    txtDescripcion->Clear();
}

void CreacionIdentificadorFrame::UpdateButtonStates(bool nuevoSelected, bool editarSelected) {
...
bool CreacionIdentificadorFrame::ConectarBaseDeDatos() {
...
void CreacionIdentificadorFrame::CerrarConexion() {
    if (conexionAbierta) {
        SQLDisconnect(hDbc);
...
void CreacionIdentificadorFrame::CargarDatosOperacion(int operacion) {
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);
    wxString query = wxString::Format(L"SELECT Operacion, OperacionID, Descripcion, UsuarioID, Ordenador FROM Identificador WHERE Operacion = %d;", operacion);
    SQLRETURN ret = SQLExecDirect(hStmt, (SQLWCHAR*)query.wc_str(), SQL_NTS);
...
bool CreacionIdentificadorFrame::ValidateInputs() {
