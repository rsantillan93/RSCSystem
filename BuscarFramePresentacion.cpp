...
wxBEGIN_EVENT_TABLE(BuscarFramePresentacion, wxFrame)
EVT_BUTTON(wxID_OK, BuscarFramePresentacion::OnBuscar)
EVT_BUTTON(wxID_CANCEL, BuscarFramePresentacion::OnCancelar)
EVT_CLOSE(BuscarFramePresentacion::OnClose)
EVT_GRID_CELL_LEFT_DCLICK(BuscarFramePresentacion::OnGridCellDClick)
EVT_GRID_SELECT_CELL(BuscarFramePresentacion::OnGridSelectCell)
EVT_CHAR_HOOK(BuscarFramePresentacion::OnKeyDown)  // Capturar la tecla ESC
wxEND_EVENT_TABLE()

BuscarFramePresentacion::BuscarFramePresentacion(wxWindow* parent, const wxString& title, const wxPoint& pos, const wxSize& size)
...
BuscarFramePresentacion::~BuscarFramePresentacion() {
    CerrarConexion();
}

void BuscarFramePresentacion::OnBuscar(wxCommandEvent& event)
{
...
void BuscarFramePresentacion::OnCancelar(wxCommandEvent& event) {
...
void BuscarFramePresentacion::OnClose(wxCloseEvent& event) {
    // Deshabilitar temporalmente el manejador de eventos para evitar ciclos o múltiples llamadas
    this->Unbind(wxEVT_CLOSE_WINDOW, &BuscarFramePresentacion::OnClose, this);
    ...
    if (!IsBeingDeleted()) {
        ...
        if (mainFrameParent != nullptr && !mainFrameParent->IsBeingDeleted()) {
            CreacionPresentacionFrame* creacionFrame = dynamic_cast<CreacionPresentacionFrame*>(mainFrameParent);
...
void BuscarFramePresentacion::OnKeyDown(wxKeyEvent& event)
{
...
void BuscarFramePresentacion::OnGridCellDClick(wxGridEvent& event)
{
    ...
    // Cast a la ventana principal (CreacionPresentacionFrame) y pasar los valores seleccionados
    CreacionPresentacionFrame* creacionFrame = dynamic_cast<CreacionPresentacionFrame*>(mainFrameParent);
    if (creacionFrame != nullptr)
    {
        creacionFrame->BuscarOperacionPresentacion(operacionID, descripcion);  // Pasar OperacionID y Descripción
...
void BuscarFramePresentacion::ActualizarEstadoBotonesNavegacion(CreacionPresentacionFrame* creacionFrame) {
    if (creacionFrame != nullptr) {
...
void BuscarFramePresentacion::OnGridSelectCell(wxGridEvent& event)
{
    int row = event.GetRow();
    UpdateRowLabels(row);
    event.Skip();
...
void BuscarFramePresentacion::UpdateRowLabels(int selectedRow)
{
...
bool BuscarFramePresentacion::ConectarBaseDeDatos()
{
...
void BuscarFramePresentacion::BuscarOperacionesEnBaseDeDatos(const wxString& operacion, const wxString& descripcion)
{
    SQLHSTMT hStmt = NULL;
    SQLAllocHandle(SQL_HANDLE_STMT, hDbc, &hStmt);

    // Construir la consulta SQL
    wxString query = L"SELECT Operacion, OperacionID, Descripcion FROM Presentacion WHERE 1=1";
...
void BuscarFramePresentacion::CerrarConexion() {
